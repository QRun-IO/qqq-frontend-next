...
layer. Tests: `src/lib/utils/sanitize-html.test.ts`; acceptance SEC-038 (a stored form,
...
`QApplicationJavalinServerTest`; acceptance SEC-034 (every document type, hashes match the
served document, framing refused, injected inline script and handler refused and reported),
SEC-035 (OAUTH2 and AUTH_0 sign-in under the policy, other origins refused), SEC-036 (the
...
`src/lib/auth/auth-storage.test.ts`; acceptance SEC-039.
...
anything else. Tests: `src/lib/auth/callback-errors.test.ts`; acceptance SEC-037.
...
- `QQQ_ACCEPTANCE_BROWSERS=chromium,firefox,webkit,mobile node scripts/acceptance.mjs`:
  ...
  area. Every new row (SEC-034 to SEC-040) passed in every project that runs it.
