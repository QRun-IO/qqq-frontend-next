...
/**
 * Renders grouped results ("Pages", "Recently viewed") as a listbox of options.
 *
 * @param props - Component properties.
 * @returns A `role="listbox"` element, with an empty message when nothing matches.
 */
export function NavigationSearchResults({ id, items, term, selectedIndex, onSelect, onHover }: NavigationSearchResultsProps) {
  ...
  return (
    <div id={id} role="listbox" aria-label="Search results" className="max-h-[400px] overflow-y-auto py-1" data-qqq-id="search-results">
      {items.length === 0 && (
        <p className="px-3 py-6 text-center text-sm text-muted-foreground" data-qqq-id="search-results-empty">
          No pages or recent records match &ldquo;{term.trim()}&rdquo;
        </p>
      )}
