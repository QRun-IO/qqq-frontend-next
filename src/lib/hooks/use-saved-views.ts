...
import type { QInstance, QJobResponse } from '@/types'
...
export async function runSavedViewProcess(processName: string, values: Record<string, unknown>): Promise<SavedView[]> {
  const response: QJobResponse & { type?: string } = await processInit(processName, { values, stepTimeoutMillis: 60 * 1000 })
  if ('error' in response && response.error) throw new Error(response.userFacingError ?? response.error)
  if (!('values' in response)) throw new Error('The saved view request did not complete.')
