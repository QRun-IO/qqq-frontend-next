...
describe('sharing API', () => {
  ...
  it('lists shares from the getSharedRecords process', async () => {
    initMock.mockResolvedValue({ processUUID: 'p', values: { resultList: [{ values: { shareId: 4, scopeId: 'READ_ONLY', audienceType: 'user', audienceId: 'sample:bob', audienceLabel: 'Bob' } }] } } as unknown as ProcessResponse)
  ...
  it('adds, edits and deletes shares through their processes', async () => {
    initMock.mockResolvedValue({ processUUID: 'p', values: {} } as unknown as ProcessResponse)
  ...
  it('raises the backend error from a failed share', async () => {
    initMock.mockResolvedValue({ processUUID: 'p', error: 'You are not the owner of this record, so you may not share it.' } as unknown as ProcessResponse)
