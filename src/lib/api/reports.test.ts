...
describe('reports API', () => {
  ...
  it('runs the report process with the report name and format', async () => {
    initMock.mockResolvedValue({ processUUID: 'p1', nextStep: 'accessReport', values: { downloadFileName: 'Owned Report - 2026.csv', serverFilePath: '/tmp/r.csv' } } as unknown as QJobResponse)
  ...
  it('returns the input fields when the report declares them, and submits them to the input step', async () => {
    initMock.mockResolvedValue({ processUUID: 'p2', nextStep: 'input', values: { inputFieldList: [{ name: 'minimumId', label: 'Minimum Id', type: 'INTEGER', isRequired: true }] } } as unknown as QJobResponse)
    ...
    vi.mocked(processStep).mockResolvedValue({ processUUID: 'p2', values: { downloadFileName: 'r.json', serverFilePath: '/tmp/r.json' } } as unknown as QJobResponse)
  ...
  it('polls asynchronous jobs until they finish', async () => {
    initMock.mockResolvedValue({ processUUID: 'p3', jobUUID: 'j3' } as unknown as QJobResponse)
    expect(await startReport('reports.basic', 'big', 'XLSX')).toEqual({ kind: 'running', processUUID: 'p3', jobUUID: 'j3' })
    vi.mocked(processStatus).mockResolvedValueOnce({ processUUID: 'p3', type: 'RUNNING', message: 'Generating Report' } as unknown as QJobResponse)
    expect(await pollReport('reports.basic', 'p3', 'j3')).toEqual({ kind: 'running', processUUID: 'p3', jobUUID: 'j3', message: 'Generating Report' })
    vi.mocked(processStatus).mockResolvedValueOnce({ processUUID: 'p3', values: { downloadFileName: 'big.xlsx', serverFilePath: '/tmp/big.xlsx' } } as unknown as QJobResponse)
  ...
  it('reports backend errors and permission denials', async () => {
    expect(reportStateFromResponse({ processUUID: 'p', error: 'Error running report', userFacingError: 'Bad input' } as QJobResponse)).toEqual({ kind: 'error', message: 'Bad input' })
    ...
    expect(reportStateFromResponse({ processUUID: 'p', values: {} } as unknown as QJobResponse)).toEqual({ kind: 'error', message: 'The report did not produce a file.' })
  })
