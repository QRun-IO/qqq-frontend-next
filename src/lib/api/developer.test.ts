...
/**
 * The form data posted to a mocked `apiClient.post` call, as plain entries.
 * @param call - Arguments of the call.
...
describe('Developer API', () => {
  ...
  describe('getTableApis', () => {
    it('reads /apis.json beside /qqq/v1 with the table name and keeps well-formed APIs', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/apis.json', { baseURL: LEGACY, params: { tableName: 'person' } })
    })
  ...
  describe('getApiVersions and apiTableSpecUrl', () => {
    it('reads versions.json under the API path', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/person-api/versions.json', { baseURL: LEGACY })
    })
    ...
    it('builds the per-table OpenAPI spec URL under the backend root', async () => {
      ...
      expect(apiTableSpecUrl({ name: 'person-api', path: '/person-api/', label: 'Person API' }, '2026.Q2', 'person'))
        .toBe(`${LEGACY}/person-api/2026.Q2/person/openapi.json`)
    })
  })
  ...
  describe('record developer routes', () => {
    it('loads the developer data and shapes each associated script', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/data/script%20lab/1/developer', { baseURL: LEGACY })
    ...
    it('creates an associated script with multipart contents and commit message', async () => {
      const { default: apiClient } = await import('./client')
      ...
      expect(call[0]).toBe('/data/scriptLab/2/developer/associatedScript/greetingScriptId')
      expect(formEntries(call)).toEqual({ contents: '// code', commitMessage: 'Initial version' })
      expect(call[2]).toMatchObject({ baseURL: LEGACY })
    })
    ...
    it('loads the logs of one revision', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/data/scriptLab/1/developer/associatedScript/greetingScriptId/102/logs', { baseURL: LEGACY })
    })
  })
