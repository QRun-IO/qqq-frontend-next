...
export async function getEsbForTable(tableName: string): Promise<EsbTableResponse | null> {
  let data: unknown
  try {
    data = await apiClient.get<unknown>(`/esb/table/${encodeURIComponent(tableName)}`)
  } catch (error) {
    const status = getErrorStatusCode(error)
    if (status === 403 || status === 404) return null
    throw error
  }
  return isEsbTableResponse(data) ? data : null
}

/**
 * Whether a response body has the shape of an ESB table response.
 *
 * @param data - The parsed response body.
 * @returns True for an object with `publications` and `subscribers` arrays.
 */
function isEsbTableResponse(data: unknown): data is EsbTableResponse {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return false
  const body = data as Partial<Record<keyof EsbTableResponse, unknown>>
  return Array.isArray(body.publications) && Array.isArray(body.subscribers)
}
