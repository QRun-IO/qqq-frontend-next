 ...
 *
 * The API catalog (`/apis.json`, `{api.path}versions.json`) and the record developer
 * routes (`/data/{table}/{pk}/developer...`) live beside `/qqq/v1`, like the legacy
 * CRUD routes. Script revisions are stored and tested through the `storeScriptRevision`
 * and `testScript` processes, as the Material dashboard does.
 */
...
/** Response of `GET /data/{table}/{pk}/developer`. */
...
/**
 * Preserve the configured host and deployment prefix for routes beside `/qqq/v1`.
...
/**
 * Path of a record's developer route.
...
function developerPath(tableName: string, primaryKey: string | number): string {
  return `/data/${encodeURIComponent(tableName)}/${encodeURIComponent(String(primaryKey))}/developer`
}
...
export async function getTableApis(tableName: string): Promise<ApiInstance[]> {
  let body: unknown
  try {
    body = await apiClient.get<unknown>('/apis.json', { baseURL: legacyBaseURL(), params: { tableName } })
  } catch (error) {
...
export async function getApiVersions(apiPath: string): Promise<ApiVersions> {
  const body = await apiClient.get<unknown>(`${apiPath}versions.json`, { baseURL: legacyBaseURL() })
...
export function apiTableSpecUrl(api: ApiInstance, version: string, tableName: string): string {
  return `${legacyBaseURL() ?? ''}${api.path}${version}/${tableName}/openapi.json`
}

/**
 * Loads a record's developer data (`GET /data/{table}/{pk}/developer`).
 *
 * @param tableName - Exact backend table identifier.
 * @param primaryKey - Record identifier.
...
export async function getRecordDeveloperData(tableName: string, primaryKey: string | number): Promise<RecordDeveloperData> {
  const body = await apiClient.get<unknown>(developerPath(tableName, primaryKey), { baseURL: legacyBaseURL() })
 ...
 * (`POST /data/{table}/{pk}/developer/associatedScript/{fieldName}`).
 *
 * @param tableName - Exact backend table identifier.
 * @param primaryKey - Record identifier.
 ...
 * (`GET .../developer/associatedScript/{fieldName}/{scriptRevisionId}/logs`).
 *
 * @param tableName - Exact backend table identifier.
 * @param primaryKey - Record identifier.
 * @param fieldName - Associated-script field.
