...
test.describe('Processes added to every query and record screen', () => {
  ...
  test('[PRC-048] the instance metadata names the process; it has no table and is in no app', async ({ backend, diagnostics }) => {
    void diagnostics
    ...
    expect(full.processes[TAG]).toMatchObject({ label: 'Tag Records', hasPermission: true })
    expect(full.processes[TAG].tableName ?? null).toBeNull()
    const inApps = JSON.stringify(full.appTree).includes(`"name":"${TAG}"`)
    expect(inApps).toBe(false)
  })

  test('[PRC-048] the query Actions menu of an unrelated table runs it on the selected rows', async ({ page, backend, diagnostics }) => {
    void diagnostics
    const inits: string[] = []
    page.on('request', (request) => { if (request.url().includes(`/processes/${TAG}/init`)) inits.push(request.postData() ?? '') })
    ...
    expect(inits.some((body) => body.includes('name="tableName"') && body.includes('person'))).toBe(true)
    expect(await tagLog(backend.sql)).toEqual([{ table_name: 'person', record_id: '1' }, { table_name: 'person', record_id: '3' }])
  })

  test('[PRC-048] the record actions menu of a table with no processes of its own runs it on that record', async ({ page, backend, diagnostics }) => {
    void diagnostics
...
test.describe('Processes added to every screen, on a phone', () => {
  ...
  test('[PRC-048] the record action sheet runs it on that record @mobile', async ({ page, backend, diagnostics }) => {
    void diagnostics
...
test.describe('Processes added to every screen, for a user whose metadata omits it', () => {
  test('[PRC-048] it is not offered on query or record screens', async ({ page, backend, diagnostics }) => {
    void diagnostics
    const full = await (await backend.api.get('/metaData')).json()
    expect(full.supplementalInstanceMetaData.materialDashboard.processNamesToAddToAllQueryAndViewScreens).toEqual([TAG])
...
test.describe('Processes added to every screen, without process permission', () => {
  ...
  test('[PRC-048] it is not offered on query or record screens and the backend refuses it', async ({ page, backend, diagnostics }) => {
    void diagnostics
