...
test('[REC-050] #audit opens the audit history and closing it clears the hash', async ({ page, backend, diagnostics }) => {
  void diagnostics
  void backend
...
test('[REC-050] #/launchProcess= opens the process run for the record and returns to it', async ({ page, backend, diagnostics }) => {
  void diagnostics
  void backend
...
test('[REC-050] a section anchor selects that section tab and scrolls to it', async ({ page, backend, diagnostics }) => {
  void diagnostics
  void backend
...
test('[REC-051] a #/createChild= link creates a child of the record with the locked join value', async ({ page, backend, diagnostics }) => {
  void diagnostics
  const writes = recordRequests(page, '/data/pet')
...
test('[REC-051] create page presets from #/defaultValues= and #/disabledFields= are filled, locked and saved', async ({ page, backend, diagnostics }) => {
  void diagnostics
