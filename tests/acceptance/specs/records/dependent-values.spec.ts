...
const ITEM_SEARCH = '/data/recPvPick/possibleValues/itemId'
...
/** The form values a captured possible-value search posted as its `values` JSON. */
...
test('[REC-052] create: Item choices follow the chosen Category and the chosen id is saved', async ({ page, backend, diagnostics }) => {
  ...
  expect(new URL(searches.at(-1)!.url()).searchParams.get('searchTerm')).toBe('Car')
...
test('[REC-052] the backend filters searches and id lookups by the posted form values', async ({ backend, diagnostics }) => {
  ...
  const search = (values: Record<string, unknown> | null, query = '') =>
    backend.api.post(`${ITEM_SEARCH}${query}`, values ? { form: { values: JSON.stringify(values) } } : {})

  // Unfiltered, the source has every item; the field's filter narrows it to the posted Category.
  expect(await labels(await backend.api.get('/possibleValues/recPvItem'))).toEqual(['Apple', 'Banana', 'Carrot', 'Leek'])
