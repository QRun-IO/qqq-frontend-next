...
import {
  VIEWER, control, fieldValue, multipartFields, openForm, openRecord, recordIdFromUrl, recordRequests, sqlCount, sqlOne, toasts,
} from './helpers'
...
test('[REC-014] STRING and TEXT editors round-trip single and multi-line text', async ({ page, backend, diagnostics }) => {
  void diagnostics
  await openForm(page, '/app/fieldLab/create', 'Create Field Lab')
...
test('[REC-017] DATE and TIME editors round-trip values and clearing', async ({ page, backend, diagnostics }) => {
  void diagnostics
  await openForm(page, '/app/fieldLab/create', 'Create Field Lab')
...
test('[REC-019] fixed-zone and per-record-zone date-times display in their declared zones', async ({ page, backend, diagnostics }) => {
  void diagnostics
...
test('[REC-020] HTML field edits in the rich-text editor and renders sanitized', async ({ page, backend, diagnostics }) => {
  void diagnostics
  await openForm(page, '/app/fieldLab/create', 'Create Field Lab')
...
test('[REC-022] BLOB upload stores the bytes and the view downloads them', async ({ page, backend, diagnostics }) => {
  void diagnostics
  await openForm(page, '/app/fieldLab/create', 'Create Field Lab')
  ...
  await page.locator('[data-qqq-id="field-value-blobValue-download"]').click()
...
test('[REC-023] dynamic defaults fill the user and dates; NONE stays empty', async ({ page, backend, diagnostics }) => {
  void diagnostics
...
test('[REC-024] max-length policies truncate, add an ellipsis, pass through or reject', async ({ page, backend, diagnostics }) => {
...
test('[REC-025] case and whitespace normalization is applied on save', async ({ page, backend, diagnostics }) => {
  void diagnostics
...
test('[REC-026] numeric range policies reject or clip as declared', async ({ page, backend, diagnostics }) => {
  for (const pattern of FIELD_LAB_ERRORS) diagnostics.allow(pattern)
  const before = await sqlCount(backend, 'select count(*) as n from field_lab')
...
test('[REC-048] a server validation error is shown once with the backend message and keeps the form', async ({ page, backend, diagnostics }) => {
  for (const pattern of FIELD_LAB_ERRORS) diagnostics.allow(pattern)
