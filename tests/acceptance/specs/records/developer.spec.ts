...
/** Stores a revision of script 101 through the v1 backend process route, as the editors do. */
async function storeRevision(backend: Backend, contents: string, commitMessage: string) {
  return backend.api.post('/qqq/v1/processes/storeScriptRevision/init', {
    multipart: { values: JSON.stringify({ scriptId: String(SCRIPT_ID), commitMessage, fileNames: 'Script.js', 'fileContents:Script.js': contents }), stepTimeoutMillis: '60000' },
  })
}

/** Runs the testScript process for script 101 with the given code and name, through the v1 route. */
async function runTest(backend: Backend, contents: string, name: string) {
  return backend.api.post('/qqq/v1/processes/testScript/init', {
    multipart: { values: JSON.stringify({ scriptId: String(SCRIPT_ID), fileNames: 'Script.js', 'fileContents:Script.js': contents, name }), stepTimeoutMillis: '60000' },
  })
}

/** The process values a captured v1 process init posted (its `values` JSON field). */
function initValues(request: Request): Record<string, unknown> {
  return JSON.parse(multipartFields(request).values ?? '{}') as Record<string, unknown>
}
...
test.describe('record developer view: associated scripts', () => {
  ...
  test('[REC-054] a table without associated scripts shows none @mobile', async ({ page, backend, diagnostics }) => {
    ...
    const developer = page.waitForResponse((reply) => new URL(reply.url()).pathname === '/qqq/v1/table/recordLab/1/developer')
  ...
  test.describe('as a viewer', () => {
    ...
    test('[REC-054] a viewer is offered no Create Script and the backend refuses creating one @mobile', async ({ page, backend, diagnostics }) => {
      ...
      const refused = await backend.api.post(`/qqq/v1/table/scriptLab/2/developer/associatedScript/${FIELD}`, {
        data: { contents: NEW_SCRIPT_CODE, commitMessage: 'Initial version' },
      })
