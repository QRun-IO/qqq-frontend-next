...
test('[REC-049] the record header keeps the title readable and wraps its controls on phones and tablets @mobile', async ({ page, diagnostics }) => {
  void diagnostics
...
test('[REC-050] record forms fit phones and tablets with touch-sized editors of every type @mobile', async ({ page, backend, diagnostics }) => {
  void diagnostics
...
test.describe('on a phone', () => {
  ...
  test('[REC-049] the phone action sheet fits the screen, keeps focus inside and returns it @mobile', async ({ page, diagnostics }) => {
    void diagnostics
    await openRecord(page, 'recordLab', 1, 'Lab: Alpha')
