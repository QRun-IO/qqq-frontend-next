...
test('[REC-034] FILE_DOWNLOAD links to the stored file under its declared name @mobile', async ({ page, backend, diagnostics }) => {
  ...
  await expect(shown(page, '[data-qqq-id="field-value-attachment-open"]')).toHaveAttribute('href', '/data/recordLab/1/attachment/alpha.txt')
  await expect(shown(page, '[data-qqq-id="field-value-attachment-download"]')).toHaveAttribute('href', '/data/recordLab/1/attachment/alpha.txt?download=1')
  await expect(fieldValue(page, 'notesFile')).toContainText('Record 1 Notes')
  await expect(shown(page, '[data-qqq-id="field-value-notesFile-open"]')).toHaveAttribute('href', '/data/recordLab/1/notesFile/Record%201%20Notes')
