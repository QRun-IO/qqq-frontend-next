...
test.describe('dashboard security headers', () => {
  test('[SEC-034] every dashboard document gets the strict policy, which allows exactly its own inline scripts', async ({ page, backend, diagnostics }) => {
  ...
  test('[SEC-034] another web site cannot frame the dashboard', async ({ page, backend, diagnostics }) => {
    void backend
  ...
  test('[SEC-034] script injected into a page is refused and reported', async ({ page, backend, diagnostics }) => {
    void backend
    allowExternalQuickSightWidget(diagnostics)
  ...
  test('[SEC-036] origins the application configures are allowed and nothing else: QuickSight embed, images, audio and custom components', async ({ page, backend, diagnostics }) => {
...
test.describe('security review fixes (QRun-IO/qqq#696)', () => {
  ...
  test('[SEC-038] stored HTML cannot add forms, style sheets or page-covering overlays', async ({ page, backend, diagnostics }) => {
    void diagnostics
  ...
  test('[SEC-039] a different user signing in on the same browser never sees the previous user\'s recent records', async ({ page, backend, diagnostics }) => {
    allowExternalQuickSightWidget(diagnostics)
  ...
  test('[SEC-040] session cookies are SameSite=Lax and Secure behind HTTPS', async ({ page, backend, diagnostics, context }) => {
    allowExternalQuickSightWidget(diagnostics)
