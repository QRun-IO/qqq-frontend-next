...
/** A displayed value of the person list: a grid cell, or a card value on a phone. */
function personValue(page: Page, value: string) {
  return grid(page, 'Person').getByRole('gridcell', { name: value, exact: true })
    .or(page.getByRole('list', { name: 'Person records' }).getByText(value, { exact: true }))
}

/** Opens the person table and waits for its rows. */
async function openPeople(page: Page) {
  await open(page, '/app/person')
  await expect(personValue(page, 'Avery')).toBeVisible()
}

test('[QRY-070] r re-queries the backend and shows the changed data @mobile', async ({ page, backend, diagnostics }) => {
  void diagnostics
  ...
  await query
  await expect(personValue(page, 'Refreshed')).toBeVisible()
  await expect(personValue(page, 'Avery')).toHaveCount(0)
...
test.describe('read-only persona', () => {
  ...
  test('[QRY-070] a user who may not insert gets no n shortcut; the backend refuses the insert @mobile', async ({ page, backend, diagnostics }) => {
    void diagnostics
    await openPeople(page)
    await page.keyboard.press('n')
