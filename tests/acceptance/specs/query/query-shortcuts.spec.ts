...
test('[QRY-065] r re-queries the backend and shows the changed data', async ({ page, backend, diagnostics }) => {
  void diagnostics
...
test('[QRY-065] f opens the filter builder and n opens the create form', async ({ page, backend, diagnostics }) => {
  void diagnostics
  void backend
  await openPeople(page)
...
test('[QRY-065] keys typed in quick search are text, not shortcuts', async ({ page, backend, diagnostics }) => {
  void diagnostics
  void backend
  await openPeople(page)
...
test.describe('read-only persona', () => {
  ...
  test('[QRY-065] a user who may not insert gets no n shortcut; the backend refuses the insert', async ({ page, backend, diagnostics }) => {
    void diagnostics
    await openPeople(page)
    await page.keyboard.press('n')
