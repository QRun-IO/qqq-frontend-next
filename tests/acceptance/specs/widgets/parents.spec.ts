...
import { expectLoaded, isLargeLayout, widget, widgetBody } from './widget-support'
...
test.describe('as administrator', () => {
  ...
  test('[WID-022] a grid parent renders each child with its own data @mobile', async ({ page, diagnostics }) => {
    void diagnostics
    ...
    if (await isLargeLayout(page)) {
      ...
      // below the large breakpoint the grid is one column: each child spans the parent, stacked
      const full = await parent.boundingBox()
      expect(Math.round(a!.width)).toBe(Math.round(full!.width))
      expect(Math.round(b!.width)).toBe(Math.round(full!.width))
      expect(Math.round(b!.x)).toBe(Math.round(a!.x))
      expect(b!.y).toBeGreaterThanOrEqual(a!.y + a!.height)
      await expectTouchReady(page, parent)
    }
  })

  test('[WID-055] a tabs parent switches tabs by click and keyboard and remembers the selection @mobile', async ({ page, diagnostics }) => {
    void diagnostics
  ...
  test('[WID-056] a parent skips an unknown child without requesting it and shows allowed children @mobile', async ({ page, backend, diagnostics }) => {
    void diagnostics
...
test.describe('as a user without the child permission', () => {
  ...
  test('[WID-056] a parent omits a denied child without requesting it @mobile', async ({ page, backend, diagnostics }) => {
    void diagnostics
